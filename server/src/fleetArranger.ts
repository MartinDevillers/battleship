...
export default class FleetArranger {
  ...
  arrange(): ShipDto[] {
    const fleet = clone(FleetArranger.DEFAULT_FLEET)
    // Naive algorithm. Just keep making random arrangements until it's valid.
    let generation = 0
    while (!this.isValidArrangement(fleet)) {
      this.randomlyArrangeFleet(fleet)
      generation++
    }
  ...
  isValidArrangement(fleet: ShipDto[]): boolean {
    const positions = fleet.flatMap((ship) => ship.positions)
